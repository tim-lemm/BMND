...
def update_result_df_optimization(results_df_opt, i, nbr_bike_lanes, nbr_none_bike_lanes, modal_share_car, modal_share_bike, index_least_used, flow_of_removed_edge):
    return pd.concat([results_df_opt,pd.DataFrame({'iteration': [int(i + 1)],
...
plot_network(edge_df, node_df, color_col_str='type_bike', base_width=1, width_scale=5, node_size=200, legend=True,
            node_label=True, title='Bike Infrastructure')
plt.show()

edge_df_results = edge_df.copy()
edge_df_results.drop(['travel_time_car','travel_time_bike','flow_car','flow_bike','length_bi'], axis=1, inplace=True)
...
while nbr_bike_lanes > 0 and i < limit:
    ...
    name_col_tt_car = 'travel_time_car' + str(i)
    name_col_tt_bike = 'travel_time_bike' + str(i)
    ...
    results_df_opt = update_result_df_optimization(results_df_opt, i, nbr_bike_lanes, nbr_none_bike_lanes, modal_share_car, modal_share_bike, index_least_used, flow_of_removed_edge)
